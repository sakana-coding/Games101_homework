...
class Bounds3
{
  public:
    ...
    inline bool
    IntersectP(const Ray& ray, const Vector3f& invDir, const std::array<int, 3>& dirisNeg) const;
};
...
{
    float tMinX = ((*this)[dirIsNeg[0]].x - ray.origin.x) * invDir.x;
    float tMaxX = ((*this)[1 - dirIsNeg[0]].x - ray.origin.x) * invDir.x;
