...
void rst::rasterizer::draw(std::vector<Triangle*>& TriangleList)
{
    ...
    for (const auto& t : TriangleList)
    {
        ...
        std::array<Eigen::Vector4f, 3> mm{
            (view * model * t->v[0]), (view * model * t->v[1]), (view * model * t->v[2])};
...
static Vector3f get_color_interpolated(float x, float y, const Triangle& t)
{
    ...
    float w_reciprocal = 1.0 / (alpha / v4[0].w() + beta / v4[1].w() + gamma / v4[2].w());
    Vector3f color_interpolated = alpha * t.color[0] / v4[0].w() + beta * t.color[1] / v4[1].w() +
                                  gamma * t.color[2] / v4[2].w();
...
static Vector3f get_normal_interpolated(float x, float y, const Triangle& t)
{
    ...
    auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
    float w_reciprocal = 1.0 / (alpha / v4[0].w() + beta / v4[1].w() + gamma / v4[2].w());
    Vector3f normal_interpolated = alpha * t.normal[0] / v4[0].w() +
                                   beta * t.normal[1] / v4[1].w() + gamma * t.normal[2] / v4[2].w();
...
static Vector2f get_texcoords_interpolated(float x, float y, const Triangle& t)
{
    ...
    auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
    float w_reciprocal = 1.0 / (alpha / v4[0].w() + beta / v4[1].w() + gamma / v4[2].w());
    Vector2f texcoords_interpolated = alpha * t.tex_coords[0] / v4[0].w() +
                                      beta * t.tex_coords[1] / v4[1].w() +
                                      gamma * t.tex_coords[2] / v4[2].w();
    texcoords_interpolated *= w_reciprocal;
    return texcoords_interpolated;
}
static Vector3f get_shadingcoords_interpolated(float x,
                                               float y,
                                               const Triangle& t,
                                               const std::array<Eigen::Vector3f, 3>& view_pos)
{
    auto v4 = t.toVector4();
    auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
    float w_reciprocal = 1.0f / (alpha / v4[0].w() + beta / v4[1].w() + gamma / v4[2].w());

    Vector3f shadingcoords_interpolated = alpha * view_pos[0] / v4[0].w() +
                                          beta * view_pos[1] / v4[1].w() +
                                          gamma * view_pos[2] / v4[2].w();

    shadingcoords_interpolated *= w_reciprocal;
    return shadingcoords_interpolated;
}
...
static float get_z_interpolated(float x, float y, const Triangle& t)
{
    ...
    auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
    float w_reciprocal = 1.0 / (alpha / v4[0].w() + beta / v4[1].w() + gamma / v4[2].w());
    float z_interpolated = alpha * v4[0].z() / v4[0].w() + beta * v4[1].z() / v4[1].w() +
                           gamma * v4[2].z() / v4[2].w();
                                         ...
                                         const std::array<Eigen::Vector3f, 3>& view_pos)
{
    ...
    int left = floor(std::min(std::min(v[0].x(), v[1].x()), v[2].x()));
    int right = ceil(std::max(std::max(v[0].x(), v[1].x()), v[2].x()));
    int below = floor(std::min(std::min(v[0].y(), v[1].y()), v[2].y()));
    int top = ceil(std::max(std::max(v[0].y(), v[1].y()), v[2].y()));



    for (int x = left; x <= right; ++x)
    {
        for (int y = below; y <= top; ++y)
        {
            if (insideTriangle(x, y, t.v))
            {
                auto interpolated_color = get_color_interpolated(x, y, t);
                auto interpolated_normal = get_normal_interpolated(x, y, t);
                auto interpolated_shadingcoords = get_shadingcoords_interpolated(x, y, t, view_pos);
                auto interpolated_texcoords = get_texcoords_interpolated(x, y, t);
                auto interpolated_z = get_z_interpolated(x, y, t);
